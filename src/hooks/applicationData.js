...
const daysApi = () => {
  return axios.get(urlDays).then((results) => {
    return results;
  });
};
const appointmentsApi = () => {
  return axios.get(urlAppointments).then((results) => {
    return results;
  });
};
const interviewersApi = () => {
  return axios.get(urlInterviewers).then((results) => {
    return results;
  });
};
...
export default function useApplicationData() {
  ...
  useEffect(() => {
    Promise.all([daysApi(), appointmentsApi(), interviewersApi()]).then(
      (results) => {
        setState((prev) => ({
          ...prev,
          days: results[0].data,
